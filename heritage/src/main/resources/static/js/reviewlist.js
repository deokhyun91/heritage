...
categoryData.onclick = () => {
	if(searchText.value==""){
		...
		location.href = `http://localhost:8000/heritage/reviewList?category=data&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
	}			
}
...
categoryFood.onclick = () => {
	if(searchText.value==""){
		...
		location.href = `http://localhost:8000/heritage/reviewList?category=food&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
	}	
}
...
categoryCafe.onclick = () => {
	if(searchText.value==""){
		...
		location.href = `http://localhost:8000/heritage/reviewList?category=cafe&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
	}	
}
...
categoryStore.onclick = () => {
	if(searchText.value==""){
		...
		location.href = `http://localhost:8000/heritage/reviewList?category=store&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
	}	
}
...
categoryGas.onclick = () => {
	if(searchText.value==""){
		...
		location.href = `http://localhost:8000/heritage/reviewList?category=gas&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
	}	
}
...
categoryParking.onclick = () => {
	if(searchText.value==""){
		...
		location.href = `http://localhost:8000/heritage/reviewList?category=parking&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
	}	
}
...
categoryHotel.onclick = () => {
	if(searchText.value==""){
		...
		location.href = `http://localhost:8000/heritage/reviewList?category=hotel&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
	}	
}
...
searchText.onkeyup = (e) => {	
	if(e.key=="Enter"){	
		if(searchText.value==""){
			...
			location.href = `http://localhost:8000/heritage/reviewList?category=${categorySelect}&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=0`;
		}		
	}
}
...
function dataCount(reviewCode){	
    $.ajax({
        type: "POST",
        async:false,
        url: `/api/v1/review/save/data/count`,        
        data: {
...
function placeCount(reviewCode){	
    $.ajax({
        type: "POST",
        async:false,
        url: `/api/v1/review/save/place/count`,        
        data: {
			reviewCode : reviewCode
        },		  	
        success: (Response) =>{				
...
function dataSearch(data,size,index){
	...
	for(let i=1;i<textListTr.length;i++){	
		textListTr[i].onclick = () => {			
			dataCount(reviewCountCode[i-1].innerText);
			location.href = `http://localhost:8000/heritage/reviewView?category=data&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${index}&code=${data[i-1].review_code}`;
		}
	}	
	...
	if(indexSize<=30){				
		...
		for(let i=0;i<listCountLi.length;i++){
			listCountLi[i].onclick = () => {
				location.href = `http://localhost:8000/heritage/reviewList?category=data&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${i}`;
			}
		}
	}		
	else{
		...
		if(indexBackFront == 0){			
			...
			listFront.onclick = () => {	
				location.href = `http://localhost:8000/heritage/reviewList?category=data&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${30}`;		
			}			
		}
		else{
			if(((indexBackFront+1)*30) >= indexSize){
				...
				listCountLi[i].onclick = () => {	
						location.href = `http://localhost:8000/heritage/reviewList?category=data&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${listCountLi[i].innerText-1}`;							
					}
				}
				
				listBack = document.querySelector(".list_back");							
				listBack.onclick = () => {	
					location.href = `http://localhost:8000/heritage/reviewList?category=data&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${(indexBackFront-1)*30}`;			
				}						
			}
			else{
				listCount.innerHTML += '<li class="list_back">&lt;</li>';		
...
function placeSearch(data,size,index){
	...
	for(let i=1;i<textListTr.length;i++){		
		textListTr[i].onclick = () => {		
			placeCount(reviewCountCode[i-1].innerText);
			location.href = `http://localhost:8000/heritage/reviewView?category=${categorySelect}&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${index}&code=${data[i-1].review_code}`;			
		}
	}	
	...
	if(indexSize<=30){				
		...
		for(let i=0;i<listCountLi.length;i++){
			listCountLi[i].onclick = () => {
				location.href = `http://localhost:8000/heritage/reviewList?category=${categorySelect}&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${i}`;
			}
		}
	}		
	else{
		...
		if(indexBackFront == 0){			
			...
			listFront = document.querySelector(".list_front");			
			listFront.onclick = () => {	
				location.href = `http://localhost:8000/heritage/reviewList?category=${categorySelect}&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${30}`;		
			}			
		}
		else{
			if(((indexBackFront+1)*30) >= indexSize){
				...
				for(let i=1;i<listCountLi.length;i++){		
				listCountLi[i].onclick = () => {	
						location.href = `http://localhost:8000/heritage/reviewList?category=${categorySelect}&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${listCountLi[i].innerText-1}`;							
					}
				}
				
				listBack = document.querySelector(".list_back");							
				listBack.onclick = () => {	
					location.href = `http://localhost:8000/heritage/reviewList?category=${categorySelect}&type=${searchOpData}&sort=${optionSortData}&searchName=${searchText.value}&index=${(indexBackFront-1)*30}`;			
				}						
			}
			else{
				listCount.innerHTML += '<li class="list_back">&lt;</li>';		
